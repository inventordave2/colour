...
#include <string.h>
#include <ctype.h>
#include "./../werner_stoop_wregex/wregex.h"
...
size_t search_history_all(const History *hist, const char *pattern, char ***results) {
    ...
    sprintf(case_insensitive_pattern, "\\i%s", pattern);
    ...
    if (!regex) {
        fprintf(stderr, "Regex compilation failed: %s at position %d\n", wrx_error(err), err_pos);
        return 0;
    }
    ...
    *results = (char **)malloc(match_capacity * sizeof(char *));
    ...
    for (size_t i = hist->size; i-- > 0;) {
        if (!hist->entries[i]) continue; // Skip NULL entries
        int ret = wrx_exec(regex, hist->entries[i], NULL, 0); // Explicit NULL for submatches
